  ...
  return (
    <>
      ...
      <SimpleGrid cols={{ base: 1, sm: 2, md: 3 }}>
        {items.map((p) => (
          <Card key={p.slug} component={Link as any} to={`/projects/${p.slug}`} withBorder radius="lg" shadow="sm" p="lg">
            <Title order={4} mb="xs">{p.title}</Title>
            <Text c="dimmed" mb="sm">{p.summary}</Text>
            <div>{p.tags.map((t) => <Badge key={t} variant="light" mr={6}>{t}</Badge>)}</div>
